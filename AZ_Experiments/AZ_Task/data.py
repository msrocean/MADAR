...
def get_continual_az_class_data(data_dir, train=True):
    ...
    if train:
        data_dir = data_dir + '/'
        XY_train = np.load(data_dir + 'Family_AZ_Train_Transformed.npz', allow_pickle=True)
        X_tr, Y_tr = XY_train['X_train'], XY_train['Y_train']

        return X_tr, Y_tr
    else:
        data_dir = data_dir + '/'
        XY_test = np.load(data_dir + 'Family_AZ_Test_Transformed.npz', allow_pickle=True)
...
def get_ember_selected_class_data(data_dir, selected_classes, train=True):
    ...
    if train:
        all_X, all_Y = get_continual_az_class_data(data_dir, train=True)
    else:
        all_X, all_Y = get_continual_az_class_data(data_dir, train=False)
...
def get_malware_multitask_experiment(dataset_name, target_classes, init_classes,\
    ...
    if dataset_name == 'AZ':
        ...
        if tasks > num_class:
            raise ValueError(f"AZ experiments cannot have more than {num_class} tasks!")
