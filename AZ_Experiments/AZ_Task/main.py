...
task_params.add_argument('--experiment', type=str, required=False,\
                         default='AZ_Task', choices=['AZ_Class', 'AZ_Task'])
...
task_params.add_argument('--tasks', type=int, default=20, help='number of tasks')
...
def run(args, verbose=True):
    ...
    if (args.replay_config == 'ifs' or args.replay_config == 'aws') and args.ifs_option == None:
        raise ValueError("IFS and AWS requires IFS Option to choose from ['ratio', 'uniform', 'gifs', 'mix'].")
...
#     print(args)
    ...
    #print(args) 
    results_save_dir = './Submission_Task/'
