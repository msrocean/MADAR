...
def testing_aucscore(model, X_test, Y_test, batch_size, device):
    ...
    with torch.no_grad():
        for x_batch, y_batch in tqdm(testloader):
            ...
            tmp_test_acc = multiclass_acc(y_test_pred, y_batch)
    ...
    #roc_auc = roc_auc_score(correct_labels, predicted_labels, multi_class='ovo')
...
def testing_aucscore_V1(model, X_test, Y_test, batch_size, device):
    X_te = torch.from_numpy(X_test).type(torch.FloatTensor)
    y_te = torch.from_numpy(Y_test).type(torch.FloatTensor)
    ...
    with torch.no_grad():
        for x_batch, y_batch in tqdm(testloader):
            ...
            for i in y_test_pred:
                ...
                normal_i = [np.float64(j)/sum(pre_normal_i) for j in pre_normal_i]
                #print(sum(normal_i))
    ...
    #print(len(y_true_list), len(y_predicts_scores))
    print(roc_auc_score(np.array(y_true_list), y_predicts_scores, multi_class="ovr", average='weighted'))
    ...
    #print(cls_report)
    #rocauc_score = roc_auc_score(np.array(y_true_list), np.array(y_predicts_scores), multi_class="ovr", average="macro")
