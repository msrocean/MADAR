...
for exp in exp_seeds:
    ...
    for task_year in range(len(all_task_years)):
        ...
        if task_year == restartMonth -1:
            tmpcurrent_task = all_task_years[task_year - 1]
            tmp_model_save_dir = str(expSaveDir) + 'model_' +\
                    str(exp_type) + str(expSaveFile) +\
                    str(memory_budget) + '/' + str(tmpcurrent_task) + '/'
            
            dict_save_file = tmp_model_save_dir + 'global_family_dict_' + str(task_year) + '.npz'
            dictFile = np.load(dict_save_file)
            GFamilyDict = dictFile['dictfile']
        else:
            dict_save_file = model_save_dir + 'global_family_dict_' + str(task_year) + '.npz'
        ...
        if current_task == all_task_years[0]:
            ...
            if batch_norm == 'yes':
                ...
                if current_task != all_task_years[0]:
                    model_new = Ember_MLP_Net_V2(input_features)
