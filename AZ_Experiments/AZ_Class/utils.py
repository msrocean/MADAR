...
def v1_get_data_loader(dataset, batch_size, cuda=False, collate_fn=None, drop_last=False, augment=False):
...
def get_data_loader(dataset, batch_size, cuda=False, collate_fn=None, drop_last=False, augment=False):
    ...
    #print(f'unique_labels {unique_labels} samples_weight {len(samples_weight)}')
    #print(weight, np.unique(samples_weight), samples_weight[:5], y[:5])

    samples_weight = torch.from_numpy(samples_weight).float()
    
    #print(f'samples_weight {len(samples_weight)} ----- {samples_weight}')
