...
def get_class_grs(model, PreviousTasksData, PreviousTasksLabels,\
                  args):
    #replay_portion = 0.5
    ...
    if args.replay_config == 'frs':
        ...
        for previousTask, CurrentTaskLabels in enumerate(PreviousTasksLabels):
            for task_Y in CurrentTaskLabels:
                ...
                task_samples = get_AWS_samples(task_Y, task_samples, k_weights,\
                        args.cnt_rate,\
                        args.num_replay_sample)
        ...
        if scenario=="class":
            ...
            if task != 1:
                    ...
                    if args.replay_config == 'aws':
                        ...
                        prev_replay_X, prev_replay_Y = get_class_grs(model_new,\
                                                     ember_train, labels_per_task[task-2:task-1], args)
                        
                    else:
                        prev_replay_X, prev_replay_Y = get_class_grs(model, ember_train, labels_per_task[task-2:task-1], args)
        ...
        if replay_mode=="offline" and scenario == "task":
            if args.replay_portion == 1.0 and args.replay_config == 'grs':
                ...
                if task == 1:
                    ...
                    prev_replay_X, prev_replay_Y = get_class_grs(ember_train, labels_per_task[task-2:task-1], args)
                    #print(f'prev_replay_Y {prev_replay_Y.shape} {np.unique(prev_replay_Y)}')
