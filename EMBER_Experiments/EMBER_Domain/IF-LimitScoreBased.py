...
def get_IFBased_samples(family_name, family_data, num_samples_per_malware_family):
    ...
    if len(data_X) > num_samples_per_malware_family:
        ...
        for ind, k in enumerate(scores_prediction):
            sorted_dict[k] = data_X[ind]

        sorted_samples_key = sorted(sorted_dict.items())[:num_samples_per_malware_family]

        replay_samples = []
        for k, v in sorted_samples_key:
            replay_samples.append(v)

        replay_samples = np.array(replay_samples)
    else:
        replay_samples = np.array(data_X)
...
#         X_test, Y_test = np.array(X_test, np.float32), np.array(Y_test, np.int32)        
        ...
        results_f = open(os.path.join(results_save_dir + 'IF_scoreLimit_' + str(num_samples_per_malware_family) + '_results.txt'), 'a')
