...
parser.add_argument('--replay_portion', type=float, default=1.0, required=False)
...
for exp in exp_seeds:
    ...
    for task_month in range(len(all_task_months)):
        ...
        model_save_dir = '../pjr_saved_model_' + str(exp_type) + '/PJR_random_replay_' +\
                            str(replay_portion) + '/' + str(current_task) + '/'
        create_parent_folder(model_save_dir)
        
        opt_save_path = '../pjr_saved_model_' + str(exp_type) + '/PJR_random_replay_' +\
                            str(replay_portion) + '/' + str(current_task) + '/'
        create_parent_folder(opt_save_path)
        
        results_save_dir =  './saved_results_' + str(exp_type) + '/PJR_random_replay_' + str(replay_portion) + '/' 
        create_parent_folder(results_save_dir)
        
        X_train, Y_train = get_task_partial_joint_training_data(data_dir, task_months, replay_portion, mlp_net=True)
        X_test, Y_test = get_task_test_data(data_dir, task_months, mlp_net=True)
        ...
        results_f = open(os.path.join('./ICDM_Domain/' + 'grs_' + str(replay_portion) + '_results.txt'), 'a')
