...
def V1_get_mistaken_stats(y_true, y_pred, Y_test_family, top_k):
    wrong_good = 0
    wrong_mal = 0
    wrong_pred = 0
    family_wrong = {}
...
def testing_aucscore_with_mistaken_stats_V1(model, X_test, Y_test, Y_test_family, batch_size, top_k, device):
    ...
    return np.mean(test_acc), roc_auc , wrong_good, wrong_mal, top_k_mistaken_families
...
def testing_aucscore(model, X_test, Y_test, batch_size, device):
    ...
    return np.mean(test_acc), precision, recall, f1score 
