...
def get_anomalyScoresSamples(fam, weight_data,\
    ...
    for ind, k in enumerate(scores_prediction):
        sorted_dict[k] = raw_data[ind]
    
    sorted_samples_key = sorted(sorted_dict.items())[:samples_in_family]
    
    mal_replay_samples = []
    for k, v in sorted_samples_key:
        mal_replay_samples.append(v)
        
    mal_replay_samples = np.array(mal_replay_samples)
...
for exp in exp_seeds:
    ...
    for task_month in range(len(all_task_months)):
        ...
        results_f = open(os.path.join(results_save_dir + layer + '_weight_final_scoreLimit_WOBN_act4_' + str(num_samples_per_malware_family) + '_results.txt'), 'a')
